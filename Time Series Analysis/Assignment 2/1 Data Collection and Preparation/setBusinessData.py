...
def start_process():
    ...
    for i in np.arange(0, len(symbols)):
        ...
        if belong_business:            
            ...
            select_q = "SELECT l.id AS location_id, ci.id AS city_id, ci.name AS city_name, s.id AS state_id, s.name AS state_name, co.id AS country_id, co.name AS country_name, ti.id AS timezone_id, ti.name AS timezone_name FROM public.location l INNER JOIN public.timezones ti ON l.timezone_id = ti.id INNER JOIN public.city ci ON l.city_id = ci.id INNER JOIN public.state s ON l.state_id = s.id  AND ci.state_id = s.id INNER JOIN public.country co ON l.country_id = co.id AND s.country_id = co.id WHERE LOWER(ci.name) = %s AND LOWER(s.abbrev) = %s"
            values = (city, state)
            location = get_rows(select_q, (city, state), ["location_id", "city_id", "city_name", "state_id", "state_name", "country_id", "country_name", "timezone_id", "timezone_name"])
            location_id = location.loc[0,'location_id']
            ...
            insert_q = "INSERT INTO business(name, location_id, sector, industry, website_url)	VALUES (%s, %s, %s, %s, %s);"
