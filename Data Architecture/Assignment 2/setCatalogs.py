...
def start_process():
    ...
    # Insert states
    # query = "INSERT INTO county(name, state_id) VALUES (%s, %s, %s);"
    columns = ['county_name', 'state_id_db']
    county = location[columns].drop_duplicates().reset_index(drop=True)
    county.columns = ['name', 'state_id']
    query = "SELECT id as county_id_db, name as county_name, state_id as state_id_db FROM county;"
    table = "county"
    location = send_return_data(location, county, columns, query, table)
    del county
    ...
    columns = ['city','county_id_db', 'timezone_id_db']
    city = location[columns].drop_duplicates().reset_index(drop=True)
    city.columns = ['name', 'county_id', 'timezone_id']
    query = "SELECT id as city_id_db, name as city, county_id as county_id_db, timezone_id as timezone_id_db FROM city;"
    table = 'city'
